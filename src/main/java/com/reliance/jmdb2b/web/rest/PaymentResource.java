...
/**
 * REST controller for managing {@link com.reliance.jmdb2b.domain.Payment}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Payment> result = paymentRepository
            ...
            .map(existingPayment -> {
                if (payment.getPgTransactionId() != null) {
                    existingPayment.setPgTransactionId(payment.getPgTransactionId());
                }
                if (payment.getClientTransactionType() != null) {
                    existingPayment.setClientTransactionType(payment.getClientTransactionType());
                }
                if (payment.getClientTransactionId() != null) {
                    existingPayment.setClientTransactionId(payment.getClientTransactionId());
                }
                if (payment.getPgStatus() != null) {
                    existingPayment.setPgStatus(payment.getPgStatus());
                }
                if (payment.getTransactioninitDate() != null) {
                    existingPayment.setTransactioninitDate(payment.getTransactioninitDate());
                }
                if (payment.getStatusUpdateDate() != null) {
                    existingPayment.setStatusUpdateDate(payment.getStatusUpdateDate());
                }
                if (payment.getAmount() != null) {
                    existingPayment.setAmount(payment.getAmount());
                }
                if (payment.getRedirectionUrl() != null) {
                    existingPayment.setRedirectionUrl(payment.getRedirectionUrl());
                }
                if (payment.getPaymentAggregator() != null) {
                    existingPayment.setPaymentAggregator(payment.getPaymentAggregator());
                }
                ...
                if (payment.getCreatedTime() != null) {
                    existingPayment.setCreatedTime(payment.getCreatedTime());
                }
                if (payment.getUpdatedTime() != null) {
                    existingPayment.setUpdatedTime(payment.getUpdatedTime());
                }
