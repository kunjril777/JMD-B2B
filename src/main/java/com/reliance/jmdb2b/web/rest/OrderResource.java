...
/**
 * REST controller for managing {@link com.reliance.jmdb2b.domain.Order}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class OrderResource {
    ...
    public ResponseEntity<Order> partialUpdateOrder(@PathVariable(value = "id", required = false) final Long id, @RequestBody Order order)
        ...
        Optional<Order> result = orderRepository
            ...
            .map(existingOrder -> {
                if (order.getCartTransactionId() != null) {
                    existingOrder.setCartTransactionId(order.getCartTransactionId());
                }
                ...
                if (order.getUpdatedTime() != null) {
                    existingOrder.setUpdatedTime(order.getUpdatedTime());
                }
