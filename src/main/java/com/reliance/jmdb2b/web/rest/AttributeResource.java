...
/**
 * REST controller for managing {@link com.reliance.jmdb2b.domain.Attribute}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Attribute> partialUpdateAttribute(
        ...
        Optional<Attribute> result = attributeRepository
            ...
            .map(existingAttribute -> {
                if (attribute.getPropetyTypeId() != null) {
                    existingAttribute.setPropetyTypeId(attribute.getPropetyTypeId());
                }
                if (attribute.getPropertyType() != null) {
                    existingAttribute.setPropertyType(attribute.getPropertyType());
                }
                if (attribute.getPropertyName() != null) {
                    existingAttribute.setPropertyName(attribute.getPropertyName());
                }
                if (attribute.getPropertyValue() != null) {
                    existingAttribute.setPropertyValue(attribute.getPropertyValue());
                }
